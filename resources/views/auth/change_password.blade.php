...
<button class="btn btn-sm btn-danger" type="reset"> Reset</button>
        ...
        <form action="{{ route('auth.change_password') }}" method="POST" enctype="multipart/form-data">
            ...
            {{ Form::inputNumber('score') }}
            {{ Form::inputDate('date') }}
            {{ Form::inputDateTime('date_time')}}
            {{ Form::inputDateRangeStart('startdate') }}
            {{ Form::inputDateRangeEnd('endDate') }}
            {{ Form::inputSelect('choice_category_id',[''=>'select']+['lucia'=>'lucia','kileo'=>'kileo']) }}
            {{ Form::inputSelect2('choice_selection_id',['lucia'=>'lucia','kileo'=>'kileo']) }}
